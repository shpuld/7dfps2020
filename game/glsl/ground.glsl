...
!!samps diffuse lightmap specular normalmap fullbright reflectmask reflectcube paletted lightmap1 lightmap2 lightmap3 deluxemap deluxemap1 deluxemap2 deluxemap3 gradient=1
...
void main ()
{
	...
	tc = v_position.xy * 0.01f;
	height = v_position.z * 0.0025f;
...
//yay, regular texture!
vec4 col = texture2D(s_diffuse, tc) * texture2D(s_gradient, vec2(0.0f, clamp(0.05f, -height + 0.5f, 0.95f)));
gl_FragColor = col;
...
//modulate that by the lightmap(s) including deluxemap(s)
	...
	#else
	vec3 lightmaps = vec3(1.3f, 1.3f, 1.3f);
	#endif
	#define deluxe vec3(0.0,0.0,1.0)
		...
		lightmaps.rgb = vec3(min(lightmaps.r, 1.3f), min(lightmaps.g, 1.3f), min(lightmaps.b, 1.3f));
